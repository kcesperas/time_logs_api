...
const STORE_CONFIG_VALIDATOR = require("../validators/store-config-validator");

// Lib declarations
const moment = require('moment');

// GET ONE RECORD
app.get('/admin/store-configs/:id', verifyAdminToken, async (req, res, next) => {
    ...
    try {
        let results = await STORE_CONFIG_MODEL.getOne(params);
...
// GET RECORDS
app.get('/admin/store-configs', verifyAdminToken, async (req, res, next) => {
    let params = await QUERY_HELPER.prepare(req);

    try {
        let results = await STORE_CONFIG_MODEL.get(params);
...
// UPDATE RECORD
app.put('/admin/store-configs/:id', verifyAdminToken, async (req, res, next) => {
    ...
    try {
        // Preparations
        params.setSql = await STORE_CONFIG_MODEL.prepareUpdate(params);
...
// CREATE RECORD
app.post('/admin/store-configs', verifyAdminToken, async (req, res, next) => {
    ...
    try {
        // Preparations
        params.insertSql = await STORE_CONFIG_MODEL.prepareSave(params);
...
// DELETE RECORD
app.delete('/admin/store-configs/:id', verifyAdminToken, async (req, res, next) => {
    ...
    try {
         // Preparations
         params.deleteSql = await STORE_CONFIG_MODEL.prepareDelete(params);
