...
let storeFulfilmentTypeSchema = require('../schemas/store-fulfilment-type-schema.json');
module.exports = {
    ...
    save: async function(params) {
        ...
        try {
            let query = `
                INSERT INTO store_fulfilment_types (${params.insertSql.INSERT}) 
                VALUES(${params.insertSql.VALUES})
                `;
            console.log('query', query, 'params.insertSql.replacements', params.insertSql.replacements);
    ...
    update: async function(params) {
        ...
        try {
            let query = `
                UPDATE store_fulfilment_types SET 
                ${params.setSql.SET}
            ...
            if( typeof results.code !== 'undefined') {
                throw new Error("Unable to perform queries. C")
            }
        } catch( error ) {
            throw new Error("Unable to perform queries. D")
        }
    ...
    prepareUpdate:  async function(params) {
        ...
        for ( colname in columns) {
            if ( !storeFulfilmentTypeSchema.updateColums.includes(colname) )
            continue;
    ...
    prepareSave:  async function(params) {
        ...
        for ( colname in columns) {
            if ( !storeFulfilmentTypeSchema.createColums.includes(colname) )
            continue;
