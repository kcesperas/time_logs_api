...
module.exports = {
    ...
    save: async function(params) {
        ...
        try {
            let query = `
                INSERT INTO brands (${params.insertSql.INSERT}) 
    ...
    update: async function(params) {
        ...
        try {
            ...
            if( typeof results.code !== 'undefined') {
                throw new Error("Unable to perform queries.A")
            }
        } catch( error ) {
            throw new Error("Unable to perform queries.B")
        }
    ...
    delete: async function(params) {
        ...
        // No results found
        if ( !results.affectedRows ) {
            let error =  new Error('No record to delete.');
            error.code = 404;
            throw error;
        }
    ...
    prepareUpdate:  async function(params) {
        ...
        for ( colname in columns) {
            if ( !brandSchema.updateColums.includes(colname) )
            continue;
    ...
    prepareSave:  async function(params) {
        ...
        for ( colname in columns) {
            if ( !brandSchema.createColums.includes(colname) )
            continue;
